...
function randomizeImgs() {
    if (backgroundOption === true) {
        ...
        let imgsArray = ["تنزيل.jpg", "تنزيل (2).jpg", "تنزيل (6).jpg", "تنزيل (9).jpg"]; // Ensure these exist in your images folder
...
let map = L.map("map", {
    ...
    scrollWheelZoom:   false,   // تعطيل الزوم بعجلة الماوس
    doubleClickZoom:   false,   // تعطيل الزوم بالضغط المزدوج
    touchZoom:         false,   // تعطيل الزوم باللمس
    keyboard:          false,   // تعطيل اختصارات لوحة المفاتيح
    boxZoom:           false,   // تعطيل الزوم بالتحديد
}).setView([30.85020703881444, 28.934246676195098], 12.5);
...
// دبابيس الأماكن
let places = [
    { name: " مدينة العالمين الجديدة", lat: 30.85579216348208, lng: 28.933497305647876 },
    { name: "ابراج مدينة العالمين الجديدة", lat: 30.866748944223385, lng:  28.918233731405007 },
    { name: "الاكاديمية العربية للعلوم والتكنولوجيا والنقل البحري", lat: 30.827672524343217,  lng: 28.87961658540151 },
];
...
tileLayer.on('tileerror', function() {
    ...
    <div style="
        ...
        justify-content:center; height:100%; background:#f0f4f8;
        border-radius:15px; color:#555; font-family:'Cairo',sans-serif;
        gap:10px;
    ">
        <i class="fa fa-map-location-dot" style="font-size:48px; color:var(--main-color)"></i>
        <p style="font-size:18px; font-weight:bold; margin:0">تعذّر تحميل الخريطة</p>
        <p style="font-size:14px; color:#888; margin:0">يرجى التحقق من اتصالك بالإنترنت</p>
    </div>
    `;
});
...
function animateCounter(el) {
    ...
    const timer = setInterval(() => {
        ...
        // عرض الأرقام الكبيرة بفواصل
        el.textContent = Math.floor(current).toLocaleString('ar-EG');
    }, 16);
}

// تشغيل العداد عند الـ Scroll باستخدام IntersectionObserver
